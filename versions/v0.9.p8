...
function draw_startup()
    ...
    -- Always draw particles via particle system
...
function draw_game()
    ...
    -- 2. Draw particles (behind ship) via particle system
    particle_sys:draw(cam_offset_x, cam_offset_y)
...
function game_manager:player_died()
    self.state = "dead"
    -- death message
    local dp = panel.new(44, 40, nil, nil, "you died!", 
        {col=8, border_style="full", shadow=true})
    self:add_panel(dp)
    
    -- continue panel
    self.continue_panel = panel.new(44, 55, nil, nil, "continue?",
        {col=11, border_style="full", pulse=true})
    self:add_panel(self.continue_panel)
end
...
function circle_event:update()
    ...
    if self.timer_panel then
        self.timer_panel:set_text(fmt2(max(0, time_left)).."S")
...
function ship:update()
    if game_manager.state == "dead" then return end
...
function ship:draw()
    -- hide player ship when dead
    if not self.is_enemy and game_manager.state == "dead" then
        return
    end
    ...
    -- player cursor - draw square above target
    if not self.is_enemy and self.target then
        local tsx, tsy = self.target:get_screen_pos()
        rect(tsx - 4, tsy - 12, tsx + 4, tsy - 4, 8)
    end
end
            ...
            if dx*dx + dy*dy < 0.5 then
                ...
                if t.hp <= 0 then
                    if t.is_enemy then
                        ...
                        -- player death - show continue panel
...
function draw_projectiles()
    for p in all(projectiles) do
        local sx, sy = cam_offset_x + (p.x - p.y) * half_tile_width, cam_offset_y + (p.x + p.y) * half_tile_height - p.z * block_h
...
function draw_arrow_to(target_x, target_y, source_x, source_y, col, orbit_dist)
    ...
    local sx, sy = cam_offset_x + (arrow_x - arrow_y) * half_tile_width, cam_offset_y + (arrow_x + arrow_y) * half_tile_height
    ...
    local arrow_size = 6
    local tip_x, tip_y = sx + cos(screen_angle) * arrow_size, sy + sin(screen_angle) * arrow_size * 0.5
