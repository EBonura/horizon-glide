...
function update_world()
    ...
    if not is_startup then
        ...
        -- Cache cleanup
        if time() - last_cache_cleanup > 8 then
...
function tile.new(x,y)
  ...
  local bsx=(x-y)*half_tile_width
  local bsy=(x+y)*half_tile_height
  local top,side,dark=get_terrain_color(h)
  local hp=(h>0) and (h*block_h) or 0

  -- one-time neighbor checks (no tiles needed)
  local face=((generate_height_at(x,y+1)<h) and 1 or 0)
           + ((generate_height_at(x+1,y)<h) and 2 or 0)
  local out=((generate_height_at(x,y-1)<=h) and 1 or 0)
          + ((generate_height_at(x-1,y)<=h) and 2 or 0)
  ...
  return setmetatable({
    ...
    r=(x+y)&1,                    -- water ripple parity
    lb=bsx-half_tile_width,       -- anchored left x
    rb=bsx+half_tile_width,       -- anchored right x
    by=bsy+half_tile_height       -- anchored mid-y (bottom of top half)
  }, tile)
end
...
function tile:draw()
  ...
  local o=self.out
  if (o&1)>0 then line(sx,sy2-half_tile_height, sx+half_tile_width, sy2, self.top_col) end -- north
  if (o&2)>0 then line(sx-half_tile_width, sy2, sx, sy2-half_tile_height, self.top_col) end -- west
end
...
tile_manager = {
    ...
    max_x = 0,
    max_y = 0,
    -- NEW: tile creation queue
    tile_queue = {},
    tiles_per_frame = 3,  -- max tiles to create per frame
}

function tile_manager:init()
    self.tiles = {}
    self.tile_list = {}
    self.tile_queue = {}  -- reset queue
    self.min_x = 0
    self.min_y = 0
    self.max_x = 0
    self.max_y = 0
...
function tile_manager:queue_tile(x, y)
    -- Only queue if not already exists and not already queued
    if not self.tiles[x] or not self.tiles[x][y] then
        -- Simple dedup: store as string key
        local key = x..","..y
        for q in all(self.tile_queue) do
            if q.key == key then return end
        end
        add(self.tile_queue, {x=x, y=y, key=key})
    end
end
...
function tile_manager:update_tiles()
    ...
    -- Queue new tiles (prioritize closer to player)
    -- Start from center and spiral out
    for dist = 0, view_range do
        for x = -dist, dist do
            for y = -dist, dist do
                -- Only check edge of current distance
                if abs(x) == dist or abs(y) == dist then
                    local wx = self.player_x + x
                    local wy = self.player_y + y
                    if wx >= new_min_x and wx <= new_max_x and
                       wy >= new_min_y and wy <= new_max_y then
                        self:queue_tile(wx, wy)
                    end
                end
            end
        end
    ...
    -- Remove tiles outside range (keep this immediate for memory)
    local to_remove = {}
...
function tile_manager:process_queue()
    -- Process up to tiles_per_frame tiles from queue
    local processed = 0
    
    -- Adaptive processing: use more budget if we have it
    local budget = self.tiles_per_frame
    if stat(1) < 0.7 then  -- If CPU < 70%, process more
        budget = self.tiles_per_frame * 2
    end
    
    while #self.tile_queue > 0 and processed < budget do
        local t = self.tile_queue[1]
        deli(self.tile_queue, 1)
        self:add_tile(t.x, t.y)
        processed += 1
    end
    
    -- Sort only if we added tiles
    if processed > 0 then
        self:sort_tiles()
    end
end
...
function tile_manager:get_height_at(x,y)
    local col=self.tiles[x]
    local t=col and col[y]
    return t and t.height or generate_height_at(x,y)
end
